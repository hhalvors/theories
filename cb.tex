...
We need to be careful here about whether the definition of a
conservative translation generalizes straightforwardly to coherent
logic. My intuition is that it does \emph{not}. In particular, for
coherent logic, we want to say something like: $F$ is conservative
means that if $F\phi (x)\vdash F\psi (x)$ then
$\phi (x)\vdash\psi (x)$. (When $F$ changes variables, we will need a
more sophisticated definition. In particular, we might want to use the
notation $\phi\vdash _{\vec{x}}\psi$ for a sequent in the context
$\vec{x}$.) For Boolean logic, the sequent $\phi (x)\vdash \psi (x)$
can be replaced with the sequent
$\vdash \forall x(\phi (x)\to \psi (x))$, and so the sentence-based
definition suffices.
...
\begin{conj} Let $C$ and $D$ be Boolean coherent categories. If
  $F:C\to D$ is conservative on $\mathrm{Sub}_C(1)$, then $F$ is
  conservative. \end{conj}

It should be the case (I think) that in a Boolean coherent (bcoh)
category $C$, for each object $X$, there is a conservative lattice
morphism $\mathrm{Sub}_C(X)\to \mathrm{Sub}_C(1)$ corresponding to the
operation of taking a relation $\phi (\vec{x})\leq \psi (\vec{x})$ to
the sentence $\forall \vec{x}(\phi (\vec{x})\to \psi (\vec{x}))$. In
other words, the claim is that if
$\forall \vec{x}(\phi (\vec{x})\to \psi (\vec{x}))$ is the maximal
subobject of $1$, then $\phi (\vec{x})\leq \psi (\vec{x})$ as
subjobjects of $X$. (It might be helpful to recall: if $f:X\to 1$ is
the unique morphism to the terminal object, and
$f^*:\mathrm{Sub}_C(1)\to \mathrm{Sub}_C(X)$ is the pullback, then $C$
Boolean implies that
$\forall _f:\mathrm{Sub}_C(X)\to \mathrm{Sub}_C(1)$ is the right
adjoint of $f^*$, and hence $\forall _f$ is conservative as a lattice
morphism.)
