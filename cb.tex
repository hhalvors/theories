...
Let $\2{Th}$ be the $2$-category of coherent theories, where the
1-cells are translations and the 2-cells are functional relations
between translations (i.e.\ t-maps). If $F:T_1\to T_2$ is a strong
translation, then $F$ is an equivalence iff $F$ is conservative and
essentially surjective. We currently lack a similar decomposition
theorem for weak translations, although we can exploit the equivalence
between $\2{Th}$ and $\2{Coh}$ to make some progress on this issue. In
particular, we know that a functor in $\2{Coh}$ is an equivalence iff
it is full, faithful, and essentially surjective.

\begin{example} We show here that for a weak translations
  ...
  \end{itemize}
  It is clear that the reason why $F$ is not an equivalence is because
  the standard conservativity condition does not rule out the
  identification of sorts that were originally not identified. In
  particular, $T_1$ does not imply any relation between $\sigma _1$
  and $\sigma _2$. Consider the $\Sigma _2$-formula
  $\chi (x,y)\equiv (x=y)$, where $x$ and $y$ are variables of sort
  $\sigma$. Then $\chi (x,y)$ is a functional relation of sort
  $F(\sigma _1)\to F(\sigma _2)$ while there is no $\Sigma _1$-formula
  that represents a functional relation of sort
  $\sigma _1\to \sigma _2$.

  The issue then seems to be that while $F$ is essentially surjective,
  it does not resemble a full functor. Intuitively, a full functor
  would have the feature that for any functional relation
  $\chi :F(\sigma _1)\to F(\sigma _2)$, there is a functional relation
  $\theta :\sigma _1\to \sigma _2$ such that $F(\theta )\simeq
  \chi$. While there is a $\Sigma _1$-formula $\eta$ such that
  $F(\eta )\simeq \chi$, this $\eta$ has the wrong arity.
...
The notion of being essentially surjective will also need to be
generalized. For example, let $T_1$ be the theory that says ``there
are exactly two things,'' and let $T_2$ be the Morita extension of
$T_1$ by the addition of a product sort. We know that the obvious
embedding $F:T_1\to T_2$ is one-half of an equivalence. However, there
are clearly formulas $\phi$ in the language of $T_2$ that are not
provably equivalent to formulas in the target of the translation
$F$. For example, let $\psi$ be the sentence that says there are four
things of the product sort. Nonetheless, there is an extended sense in
which this formula $\psi$ is in the image of $F$. Indeed, if $\chi$ is
the ``code'' for the product sort $\sigma '$, then for any
$\Sigma _2$-formula $\psi (x)$ with $x$ of type $\sigma '$, there is a
$\Sigma _1$ formula $\phi (y)$ such that
...
surjectivity'' will necessarily involve t-maps.
