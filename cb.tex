...
It should be the case (I think) that in a Boolean coherent category
$C$, for each object $X$, there is a conservative lattice morphism
$\mathrm{Sub}_C(X)\to \mathrm{Sub}_C(1)$ corresponding to the
operation of taking a relation $\phi (\vec{x})\leq \psi (\vec{x})$ to
the sentence $\forall \vec{x}(\phi (\vec{x})\to \psi (\vec{x}))$.
